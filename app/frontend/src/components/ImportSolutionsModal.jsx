...
export default function ImportSolutionsModal({ isOpen, onClose, onImport, books, chapters, selectedBookId, selectedChapterId }) {
  ...
  const validateJson = (text) => {
    ...
    // Check if it has the solutions array
    if (parsed.solutions && Array.isArray(parsed.solutions)) {
      setIsValidJson(true);
      setParsedCount(parsed.solutions.length);
    } else if (Array.isArray(parsed)) {
      // If it's just an array, wrap it
      setIsValidJson(true);
      setParsedCount(parsed.length);
    } else {
      setError('JSON must have a "solutions" array property or be an array of solutions');
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      let solutions;
      try {
        solutions = JSON.parse(jsonInput);
      } catch {
        // Try with fixed escape sequences
        solutions = JSON.parse(fixInvalidEscapeSequences(jsonInput));
      }

      // Wrap array in solutions object if needed
      ...
      setError(e.message || 'Failed to import solutions');
    } finally {
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-4 border-b bg-purple-50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-gray-800">Import Solutions</h2>
              <p className="text-sm text-gray-500">Upload or paste JSON to create a solution set</p>
            </div>
          </div>
          <button
            onClick={handleClose}
        ...
        <div className="flex-1 overflow-auto p-4 space-y-4">
          ...
          {isValidJson && (
            <div className="flex items-center gap-2 text-purple-600 bg-purple-50 p-3 rounded-lg">
              ...
              <span className="text-sm">Valid JSON - {parsedCount} solutions found</span>
            </div>
          )}
        </div>
          ...
          >
            {isSubmitting ? (
              ...
              <>
                ...
                Import {parsedCount > 0 && `(${parsedCount} solutions)`}
              </>
            )}
          </button>
        </div>
